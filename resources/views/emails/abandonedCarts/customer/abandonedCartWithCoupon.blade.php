...
<tr>
    <td style="
        ...
        <img
            width="180px"
            src="{{ config('app.url') . '/images/email/abandonedCarts/customer/icon-carrinho-abandonado.png' }}"
...
<tr>
    <td style="padding: 20px 8% 0; text-align: center; font-size: 14px; line-height: 20px;">
        ...
        <p><strong>Notamos que você quase completou a compra do {{ $product['name'] }}</strong>, mas algo ficou pendente. Que tal um cupom exclusivo para te ajudar a concluir a compra?</p>
        <p><strong>Cupom: {{ $abandonedCart->code }}</strong> <br> <strong>Validade: {{ \Carbon\Carbon::parse($abandonedCart->end_at)->format('d/m/Y H:i') }}
            </strong></p>
        <p>Não perca essa oportunidade. <br> O desconto é por tempo limitado!</p>
    </td>
</tr>
<tr>
    <td
