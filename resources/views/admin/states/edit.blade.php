    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.states.update", [$state->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="noScrollInput form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="number" name="code" id="code" value="{{ old('code', $state->code) }}" step="1">
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\State::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $state->status) === (string) $key ? 'checked' : '' }} required>
