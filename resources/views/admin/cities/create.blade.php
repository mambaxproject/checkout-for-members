...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.cities.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="photo">{{ trans('cruds.city.fields.photo') }}</label>
                <div class="needsclick dropzone {{ $errors->has('photo') ? 'is-invalid' : '' }}" id="photo-dropzone">
                </div>
                @if($errors->has('photo'))
                    <span class="text-danger">{{ $errors->first('photo') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.city.fields.photo_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="cover_photo">{{ trans('cruds.city.fields.cover_photo') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.city.fields.cover_photo_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    init: function () {
@if(isset($city) && $city->photo)
      var file = {!! json_encode($city->photo) !!}
...
<script>
    ...
    init: function () {
@if(isset($city) && $city->cover_photo)
      var file = {!! json_encode($city->cover_photo) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
      file.previewElement.classList.add('dz-complete')
